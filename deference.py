...
for even a reckless policy; as it shrinks, collision avoidance becomes the
robot's problem — the regime where the safety layer has to earn its keep.
The expected picture: the unconstrained flow policy starts colliding at low
deference, the calibrated ones don't. Writes deference.csv.
"""
    ...
    for d in DEFERENCE:
        for name, calib in [("raw      ", None),
            ...
            print(f"deference {d}: {name}: success {line['success']:.0%}, "
                  f"steps {line['steps']:.0f}, collisions {line['collisions']:.2f}, "
